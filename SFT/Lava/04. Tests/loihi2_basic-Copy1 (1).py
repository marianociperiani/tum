...
def plot_results(sft, in_data):
    ...
    # Step 3: Compute the FFT of the in_data
    fft_result = np.fft.fft(in_data)
    frequencies = np.fft.fftfreq(256)
    np.savetxt('fft_result.txt', fft_result,delimiter=' ', fmt='%10.2f')
    # Create the figure and subplots
    fig, axes = plt.subplots(2, 1, figsize=(10, 6))
    
    # Plot the original signal (in_data)
    axes[0].plot(in_data)
    ...
    plt.savefig('fft_plot.png', format='png')
    ...
    ax[0].plot(fft, color="cornflowerblue")
...
def main(N=256, T=20, SFT=True, profile=False):
    ...
    if not SFT:
        ...
        # raw_data = np.loadtxt("256-samples.txt")
